...
class YandexAPI:
    ...
    def load_stations_to_json(self):
        ...
        :rtype: None
        """
        stations_id = requests.get(
            f"https://api.rasp.yandex-net.ru/v3.0/stations_list/?apikey={self.YandexAPI_Key}&lang=ru_RU&format=json"
        )
    ...
    def station_request(
        self, id_station_from, id_station_to, date=datetime.datetime.now().strftime("%Y-%m-%d"), lang="ru_RU"
    ):
