...
def create_repeat_reservation(request):
    """
    Создание бронирования с возможностью повторения (ежедневно, еженедельно, ежемесячно).

    :param request: HTTP запрос
    :return: страница с формой или перенаправление на активные брони
    """
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logger.info(f"Пользователь {request.user.username} создал бронь #{reservation.reservation_id} с повторением {reservation.repeat_type}")
