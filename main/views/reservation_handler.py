...
def reservation_handler(request):
    ...
    if not user_id or not train_number or not seats: return JsonResponse({'status': 'error', 'message': 'Недостаточно данных для бронирования'}, status=400)
    try:
        profile = Profile.objects.get(user__id=user_id)
        logger.debug(f"Пользователь найден: #{user_id}.")
        user = profile.user
        if station_in_id: station_in = Station.objects.get(station_id=station_in_id)
        elif station_in_name: station_in, _ = Station.objects.get_or_create(name=station_in_name)
        else: return JsonResponse({'status': 'error', 'message': 'Не указана станция отправления'}, status=400)
        if station_out_id: station_out = Station.objects.get(station_id=station_out_id)
        elif station_out_name:station_out, _ = Station.objects.get_or_create(name=station_out_name)
        else: return JsonResponse({'status': 'error', 'message': 'Не указана станция назначения'}, status=400)
        ...
        if not train:
            train = Train.objects.create(id_station_start=station_in, id_station_stop=station_out,station_at_time=departure_time or '2024-01-01 00:00:00', path=json.dumps({'number': train_number})            )
    ...
    for item in seats.split('W'):
        if item and 'x' in item:
            ...
            if Reservation.objects.filter(train=train, place_num=place_num, status='active').exists():
                ...
                return JsonResponse({'status': 'error','message': f'Место {seat_num} в вагоне {carriage_num} уже занято'}, status=400)
    ...
    if profile.balance < total_price:
        ...
        return JsonResponse({'status': 'error','message': f'Недостаточно средств. Баланс: ${profile.balance.amount:.2f}, нужно: ${total_price.amount:.2f}'}, status=400)
    profile.update_balance(-total_price)
    if profile.is_vip:
        cashback = total_price.amount * Decimal('0.05')
        profile.update_balance(cashback)
    profile.save()
    reservation = Reservation.objects.create(user=user,train=train,place_num=', '.join(place_nums),station_in=station_in, station_out=station_out, status='active')
    logger.info(f"Успешное бронирование #{reservation.reservation_id}.")
    seats_list = ', '.join(place_nums)
    return JsonResponse({'status': 'success','message': f'Бронирование #{reservation.reservation_id} оформлено! Места: {seats_list}. Сумма: ${total_price.amount:.2f}','reservation_id': reservation.reservation_id,'places': place_nums,'total_price': str(total_price.amount),'new_balance': str(profile.balance.amount)}, status=200)